...
def minmax(L):
    ...
    firstElement = min(L)
    secondElement = max(L)
    returnTuple = (firstElement, secondElement)
    return returnTuple
...
def compress_vector(x):
    ...
    for i in range(0, len(x)):
        ...
        if secondElement != 0:
            ind.append(i)
            val.append(secondElement)
            print(ind)
            print(val)
        i = i + 1
    #new_d.update({ind,val})
    d['inds'] = ind
    d['vals'] = val
    #print(d)
    return d

#x = [0.0, 0.87, 0.0, 0.0, 0.0, 0.32, 0.46, 0.0, 0.0, 0.10, 0.0, 0.0,3.0]
#compress_vector(x)
...
def decompress_vector(d, n=None):
    ...
    #for i in range(0, n):
        #new_inds.append(i) #append the increment
    for i, j in zip(inds, vals):
        new_inds[i]+=j
    return new_inds
  
        
   
##d = {'inds': [4, 7, 11, 12, 15, 17], 'vals': [0.43, 0.86, 0.34, 0.47, 0.75, 0.76]}
#print(d['inds'])
#print(d['vals'])
#decompress_vector(d,n=18)
...
def find_common_inds(d1, d2):
    ...
    ### YOUR CODE HERE
    ###
    ### Create a set of d1 and d2 and compare the sets
    setD1 = set(d1['inds'])
    setD2 = set(d2['inds'])
    
    ##compare the sets using intersection
    returnList = list(setD1 & setD2)
    return returnList
